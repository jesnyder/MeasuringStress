                ...
                for segment in segment_list:
                    ...
                    for range in searchRange:
                        ...
                        if segment == 'All':
                            ...
                            find_inflections(study, format_type, record, sensor, segment, int(range))
...
def find_inflections(study, format_type, record, sensor, segment, range):
    """
...
def plot_inflections(study, record, sensor, segment):
    ...
    segment, range =  'All', searchRange[-1]
    path = [study, 'analyzed', 'inflections', 'all_times', str(range), record, segment]
    file = os.path.join(*path, sensor + ".csv")
